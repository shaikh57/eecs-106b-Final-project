...
class FishConfigurationSpace(ConfigurationSpace):
    ...
    def local_plan(self, c1, c2, dt=0.01):
        ...
        best_plan = None
        def rCTheta(theta):
            return 0.42*np.exp(-0.03819*abs(theta))
        ...
        for u1 in np.linspace(self.input_low_lims[0], self.input_high_lims[0], 17):
            for u2 in np.linspace(self.input_low_lims[1], self.input_high_lims[1], 10):
                ...
                for t in np.arange(dt, total_time, dt):
                    if u1 == 0:
                        q_dot = np.array([np.cos(c1_upd[2])*(0.0039*abs(u2*180/np.pi) - 0.0323), np.sin(c1_upd[2])*(0.0039*abs(u2*180/np.pi) - 0.0323), 0])
                    else:
                        r = rCTheta(c1_upd[2]*180/np.pi)
                        # print(u1,"\n")
                        q_dot = np.array([r*np.cos(c1_upd[2]), r*np.sin(c1_upd[2]), u1])
                ...
                if self.distance(plan.end_position(), c2) < best_dist:
                    ...
                    best_plan = plan
        # print("best_plan.position: ", best_plan.positions[-1], "\n")
        return best_plan
